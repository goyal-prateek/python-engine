...
async def generate_story(data: GenerateStoryRequest, request: Request):
    ...
    story_text, url = await StoryService.generate_story(data, shared_llm_clients=clients)
...
async def generate_story_sse(data: GenerateStoryRequest, request: Request):
    ...
    async def event_stream():
        async for event, text in StoryService.generate_story_sse(data, shared_llm_clients=clients):
